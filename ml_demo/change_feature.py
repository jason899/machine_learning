...
def revise_features(tdf):
    ...
    tdf = tdf.drop(['terminal_start_date', 'last_terminal_start_date', 'last_terminal_end_date'], axis=1)

    # group by brand
    terminal_brand = tdf.groupby('terminal_brand')
    last_terminal_brand = tdf.groupby('last_terminal_brand')

    terminal_brand_mean = terminal_brand.mean()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    terminal_brand_sum = terminal_brand.sum()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    terminal_brand_count = terminal_brand.count()[['user_id']]

    last_terminal_brand_mean = last_terminal_brand.mean()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    last_terminal_brand_sum = last_terminal_brand.sum()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    last_terminal_brand_count = last_terminal_brand.sum()[['user_id']]

    tdf = tdf.join(terminal_brand_mean, on='terminal_brand', rsuffix='_ter')
    tdf = tdf.join(terminal_brand_sum, on='terminal_brand', rsuffix='_ter')
    tdf = tdf.join(terminal_brand_count, on='terminal_brand', rsuffix='_ter')

    tdf = tdf.join(last_terminal_brand_mean, on='last_terminal_brand', rsuffix='_last_ter')
    tdf = tdf.join(last_terminal_brand_sum, on='last_terminal_brand', rsuffix='_last_ter')
    tdf = tdf.join(last_terminal_brand_count, on='last_terminal_brand', rsuffix='_last_ter')

    # terminal_brand_min = terminal_brand.min()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    # terminal_brand_max = terminal_brand.max()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    # last_terminal_brand_min = last_terminal_brand.min()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    # last_terminal_brand_max = last_terminal_brand.max()[['last_terminal_used_month', 'terminal_gap', 'terminal_user_month']]
    # tdf = tdf.join(terminal_brand_min, on='terminal_brand', rsuffix='_ter')
    # tdf = tdf.join(terminal_brand_max, on='terminal_brand', rsuffix='_ter')
    # tdf = tdf.join(last_terminal_brand_min, on='last_terminal_brand', rsuffix='_last_ter')
    # tdf = tdf.join(last_terminal_brand_max, on='last_terminal_brand', rsuffix='_last_ter')

    tdf = tdf.drop(['terminal_brand', 'last_terminal_brand', \
...
result.to_csv('./data/result/change_feature.csv')
...
parameters = {'n_estimators': [10, 100, 200, 500, 1000], 'max_features': [0.1, 0.2, 0.5, 0.9]}
